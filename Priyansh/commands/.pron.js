...
module.exports.run = async ({ api, event, args }) => {
  if (event.senderID !== api.getCurrentUserID() && !global.config.ADMINBOT.includes(event.senderID)) {
    return api.sendMessage("Sirf owner is command ko use kar sakta hai.", event.threadID, event.messageID);
  }

  const request = global.nodemodule["request"];
  const fs = global.nodemodule["fs-extra"];

  const stylishMsg = `𝗬𝗮𝗿𝗿 𝗧𝗨 𝗯𝘂𝗵𝘁 𝘁𝗵𝗿𝗸𝗶 𝗵𝘆 🥵
𝗕𝗮𝗮𝘇 𝗻𝗵𝗶 𝗮𝘆𝗲 𝗴𝗮 𝘀𝘂𝗱𝗵𝗿 𝗷𝗮
𝗞𝘂𝗰𝗵 𝗻𝗵𝗶 𝗿𝗮𝗸𝗵𝗮 𝗲𝗻 𝗸𝗮𝗺𝗼 𝗺𝗲
𝗟𝗲𝗵 𝗸𝗿 𝗿𝗵𝗮 𝗵𝘂 𝘃𝗶𝗱𝗲𝗼 𝘀𝗲𝗻𝗱`;

  api.sendMessage(stylishMsg, event.threadID, async () => {
